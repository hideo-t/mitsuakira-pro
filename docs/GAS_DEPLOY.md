...
> **ただし `gas:pull` は `gas/` の中身を GAS 側で上書きする。**
> 手元に GAS へ未反映の変更があるときに実行すると、その変更は消える。
> その場合は pull せず、別ディレクトリへ `clasp clone-script <scriptId>` して
> `appsscript.json` だけコピーし、`.gs` の差分は目視で確認すること。
> （2026-09-06 の初回導入時は実際にこの手順を取った。手元が GAS より進んでいたため）
...
`clasp push` は**プロジェクトのファイル一式を丸ごと置き換える**。
2026-09-06 の初回 push で、GAS 側にあった `コード.gs` は
`supporter-registration.gs` に置き換わった（中身は同一）。
以後エディタに並ぶのは `gas/` にあるファイルと同じ名前になる。
ローカルに無いファイルは push で消えるので、
**GAS エディタだけに新しいファイルを作らないこと。**
