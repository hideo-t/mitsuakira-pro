...
21: updated_at     - 更新日時
22: program        - 出し物（演目）
23: organizer      - 主催（空欄 = 三晶の自主公演）
24: contact        - 問い合わせ先（主催が三晶以外のときサイトに表示）
```
...
**主催（organizer）の扱い**:
`organizer` が空欄、または `三晶` / `円左衛門` / `mitsuakira` を含む場合は自主公演とみなし、
公開サイトに「申し込む」ボタンを出します。それ以外の主催名が入っている回は
申し込みを受け付けず、代わりに `contact` の内容を問い合わせ先として表示します
（`contact` 内の URL・メールアドレス・電話番号は自動でリンクになります）。

23・24列目を後から追加した既存シートには、GAS エディタで
`migrateAddOrganizerColumns()` を一度実行してヘッダを付与してください。
既存行の値は空欄のままで構いません（＝従来どおり自主公演として扱われます）。

### 過去公演（これまでの公演タブ）

公開サイトのイベント節は「今後の公演 / これまでの公演」のタブに分かれています。

- `getPublicEvents()` … 開催日が今日以降で status が open/full/published の回（申し込み用）
- `getPastEvents()` … 開催日が過ぎた回のうち status が draft 以外のもの（新しい順・表示専用）

「これまでの公演」タブは、利用者がタブを開いたときに初めて
`?action=getPastEvents` を取りに行きます（初回表示を重くしないため）。

白河移転（2026年2月10日）以降の出演履歴を投入するには、GAS エディタで
`seedPastEvents()` を一度実行してください。同じ `event_id` の行が既にある場合は
上書きせずスキップします（手入力済みの内容を壊さないため）。
